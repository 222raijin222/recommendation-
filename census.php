...
document.getElementById("addMemberBtn").addEventListener("click", () => {
  ...
  block.innerHTML = `
    <h4>Household Member #${memberCount}</h4>

    <div class="member-row">

      <div>
        <label>Name</label>
        <input type="text" name="member_name_${memberCount}" placeholder="Surname, First name, M.I.">
      </div>

      <div>
        <label>Age</label>
        <input type="number" name="age_${memberCount}" readonly>
      </div>

      <div>
        <label>Birth Month</label>
        <select name="birth_month_${memberCount}" onchange="computeAge(${memberCount})">
      ...
      <div>
        ...
        <select name="birth_year_${memberCount}" onchange="computeAge(${memberCount})">
          <option value="">Select</option>
          ${yearOptions}
        </select>
      </div>
      ...
      <div>
        ...
        <input type="text" name="philhealth_id_${memberCount}" style="display:none;">
      </div>
      ...
      <div>
        <label>PWD ID</label>
        <input type="text" name="pwd_id_${memberCount}" style="display:none;">
      </div>
      ...
      <div>
        ...
        <select name="religion_${memberCount}">
          <option value="">Select</option>
      ...
      <div>
        ...
        <select name="citizenship_${memberCount}">
          <option value="">Select</option>
      ...
      <div>
        ...
        <select name="currently_enrolled_${memberCount}" style="display:none;" onchange="handleEnrollment(${memberCount})">
          <option value="">Select</option>
          <option>Yes</option>
          <option>No</option>
        </select>
      ...
      <div>
        ...
        <input type="text" name="school_place_${memberCount}" style="display:none;">
      </div>
    ...
    <button class="remove-btn" onclick="document.getElementById('member_${memberCount}').remove()">Remove Member</button>
...
function computeAge(i) {
  const mo = document.querySelector(`[name="birth_month_${i}"]`).value;
  const yr = document.querySelector(`[name="birth_year_${i}"]`).value;
  const ageField = document.querySelector(`[name="age_${i}"]`);

  if (!mo || !yr) return ageField.value = "";

  const now = new Date();
  let age = now.getFullYear() - yr;
  if (now.getMonth() + 1 < mo) age--;
  ageField.value = age;
}

function handlePhilhealth(i) {
  const have = document.querySelector(`[name="philhealth_have_${i}"]`).value;
  const field = document.querySelector(`[name="philhealth_id_${i}"]`);
  field.style.display = have === "Yes" ? "block" : "none";
}

function handlePWD(i) {
  const have = document.querySelector(`[name="pwd_have_${i}"]`).value;
  const field = document.querySelector(`[name="pwd_id_${i}"]`);
  field.style.display = have === "Yes" ? "block" : "none";
}

function handleEducation(i) {
  const lvl = document.querySelector(`[name="education_level_${i}"]`).value;
  const enrolled = document.querySelector(`[name="currently_enrolled_${i}"]`);

  enrolled.style.display = (lvl !== "No education" && lvl !== "College graduate") ? "block" : "none";
}

function handleEnrollment(i) {
  const en = document.querySelector(`[name="currently_enrolled_${i}"]`).value;
  const level = document.querySelector(`[name="school_level_${i}"]`);
  const place = document.querySelector(`[name="school_place_${i}"]`);

  level.style.display = en === "Yes" ? "block" : "none";
  place.style.display = en === "Yes" ? "block" : "none";
}

function handleEmployment(i) {
  const st = document.querySelector(`[name="employment_status_${i}"]`).value;
  document.querySelector(`[name="work_details_${i}"]`).style.display = st === "Employed" ? "block" : "none";
  document.querySelector(`[name="monthly_income_${i}"]`).style.display = st === "Employed" ? "block" : "none";
}
</script>
...
<div class="form-section">
  ...
  <div class="form-group">
    <label for="work_place">Name of Work/Business (please write the response):</label>
    <input type="text" id="work_place" name="work_place" placeholder="Enter name of work or business">
  </div>
</div>
